...
// network2
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            if(srcIp.toString().equals("192.168.7.101")){
                if(dosCounter>=5){
                    log.info("dos found and drop this packet::"+dosCounter);
                    return;
                }
                log.info("dos not found and send this packet:"+dosCounter);
                dosCounter++;
            }else {
                log.info("dos not found and send this ip address");
            }
...
/*
            ...
            if (pkt.receivedFrom().deviceId().equals(dst.location().deviceId())) {
                if (!context.inPacket().receivedFrom().port().equals(dst.location().port())) {
                    log.info("final last switchchchch jadue");
        ...
        if (ipv4Protocol1 == IPv4.PROTOCOL_UDP) {
            ...
            if (ipv4Packet1 instanceof IPv4) {
                    ...
                    if (payload instanceof UDP && ((UDP) payload).getDestinationPort()== 135)
                         {
                        if(dstIPAddress.toString().equals("192.168.7.105")){
                             ...
                             packetService.emit(packetOut);
                        ...
                        else if(dstIPAddress.toString().equals("192.168.0.205")){
                                // set src and destination IP address
                                ...
                                newEthPkt.setPayload(ipv4Packet1.getPayload());
                                ByteBuffer newPktBuffer = ByteBuffer.wrap(newEthPkt.serialize());
                                TrafficTreatment treatment = DefaultTrafficTreatment.builder().setOutput(PortNumber.portNumber(4)).build();
                                OutboundPacket packetOut = new DefaultOutboundPacket(currentDeviceId, treatment, newPktBuffer);
                                packetService.emit(packetOut);
                        }
